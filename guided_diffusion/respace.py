...
from .diff.gaussian_diffusion_enhance  import GaussianDiffusion  #train/test WA with diffusion
#from .diff.gaussian_diffusion import GaussianDiffusion #train/test pure diff
...
#     def __call__(self, x, ts, light_factor=1, light_mask=1, corner=1, **kwargs):
#         map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
#         new_ts = map_tensor[ts]
#         if self.rescale_timesteps:
#             new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
...
#             return self.model(x, new_ts, **kwargs)
#         else:
#             return self.model(x, new_ts, light_factor, light_mask, corner,y=1)